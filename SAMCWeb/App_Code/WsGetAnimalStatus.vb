Imports System.Data
Imports System.Web
Imports System.Web.Services
Imports System.Web.Services.Protocols
Imports MySql.Data.MySqlClient

' To allow this Web Service to be called from script, using ASP.NET AJAX, uncomment the following line.
<System.Web.Script.Services.ScriptService()>
<WebService(Namespace:="http://tempuri.org/")> _
<WebServiceBinding(ConformsTo:=WsiProfiles.BasicProfile1_1)> _
<Global.Microsoft.VisualBasic.CompilerServices.DesignerGenerated()> _
Public Class WsGetAnimalStatus
    Inherits System.Web.Services.WebService

    Dim DBConn As New MySqlConnection(ConfigurationManager.ConnectionStrings("samc_db").ConnectionString)

    <WebMethod()>
    Public Function GetAnimalStatus() As List(Of ListItem)

        Dim _ListOfPetStatus As New List(Of ListItem)()
        Dim DtPetStatus As New DataTable
        Dim ClsAnimal As New ClsAnimal

        Try
            DtPetStatus = ClsAnimal.GetAnimalStatus(ClsAnimal, DBConn)

            If DtPetStatus.Rows.Count > 0 Then

                For i As Integer = 0 To DtPetStatus.Rows.Count - 1

                    _ListOfPetStatus.Add(New ListItem() With {
                        .Value = DtPetStatus.Rows(i).Item("StatusCode").ToString,
                        .Text = DtPetStatus.Rows(i).Item("StatusName").ToString
                    })

                Next

            End If

        Catch ex As Exception

        End Try

        Return _ListOfPetStatus

    End Function

    Public Function HelloWorld() As String
        Return "Hello World"
    End Function

End Class